...
class AIProvider:
    ...
    def check_tool_calling_support(self) -> Dict[str, Any]:
        result = {
            'supported': False,
        ...
        # Create a test message that should trigger the tool
        test_messages = [
            {
                "role": "system",
                "content": "You are a helpful assistant. When the user asks you to call the test function, you MUST call it using the test_function tool."
            },
            {
                "role": "user",
                "content": "Please call the test function with message 'hello'."
            }
        ]

        try:
            # Test at temperature=0.0 (what Minerva chat uses for deterministic retrieval)
            ...
            if tool_calls:
                # Model supports tool calling at temp 0.0
                result['supported'] = True
            else:
                # Model didn't use tool calls
                result['supported'] = False
                if content and ('test_function' in content.lower() or 'function' in content.lower()):
                    result['warning'] = (
                        f"Model '{self.llm_model}' does not support function/tool calling.\n"
                        f"  The model returned tool information as text instead of using tool calls.\n"
                        f"  Chat features may not work properly with this model."
                    )
                else:
                    result['warning'] = (
                        f"Model '{self.llm_model}' may not support function/tool calling.\n"
                        f"  Unable to verify tool calling capability.\n"
                        f"  Chat features may not work properly with this model."
                    )

        except Exception as error:
            result['supported'] = False
            result['error'] = f"Failed to check tool calling support: {error}"
