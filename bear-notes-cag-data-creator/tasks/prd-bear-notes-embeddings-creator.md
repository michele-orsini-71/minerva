...
2. **Intelligent Chunking**: The system must split note content into chunks while:
   - Preserving code blocks and tables as atomic units (no splitting)
   - Respecting semantic boundaries (headings, paragraphs, sentences)
   - Using character-based sizing (~4 characters ≈ 1 token, since Ollama's mxbai-embed-large does not expose a tokenizer)
   - Using optimized chunk size determined through testing methodology (see requirement 11)
   - Implementing 10-20% overlap between consecutive chunks
...
6. **Command Line Interface**: The system must provide a CLI that accepts:
   ...
   - `--chunk-size` option (default: 1200 characters)
   - `--overlap-percent` option (default: 15%)
   - `--chromadb-path` option (default: `../chromadb_data/bear_notes_embeddings`)
...
9. **Comprehensive Test Coverage**: The system must include automated tests for each pipeline phase:
   ...
   **Chunking Algorithm Tests (CRITICAL - Comprehensive Coverage Required):**
   - **Code Block Preservation**: Test that fenced code blocks (```), indented code blocks, and inline code spans are never split across chunks
   - **Table Integrity**: Verify markdown tables remain complete within single chunks
   - **Heading Boundary Respect**: Test that chunks break at heading boundaries and carry heading context
   - **Paragraph Preservation**: Ensure paragraphs are not split mid-sentence
   - **Character-based Sizing**: Test chunk size calculations using ~4 characters per token approximation
   - **Overlap Calculation**: Verify 10-20% overlap is calculated correctly at character boundaries
...
- Sensible defaults based on research (1200-2000 characters ≈ 300-500 tokens, 10-20% overlap)
